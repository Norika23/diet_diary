 ...
 */
 class Follow extends User{
	...
	public function checkFollow($followerID, $user_id){
		...
		$stmt = $database->query("SELECT * FROM `follow` WHERE `sender` = $user_id  AND `receiver` = $followerID");
	...
	public function whoToFollow($user_id, $profileID){
		global $database;
		$stmt = $database->query("SELECT * FROM `users` WHERE `id` != $user_id AND `id` NOT IN (SELECT `receiver` FROM `follow` WHERE `sender` = $user_id) ORDER BY rand() LIMIT 3");
		$users = $stmt;
			...
			echo '<div class="follow-body">
					...
					<div class="follow-content">
						<div class="fo-co-head">
							<a href="'.BASE_URL.$user['username'].'">'.$user['username'].'</a><span>@'.$user['username'].'</span>
						</div>
						<!-- FOLLOW BUTTON -->
						'.$this->followBtn($user['id'], $user_id, $profileID).'
					</div>
				</div>';
		}
		echo '</div></div>';
